...
def combine_prescorer_scorer_results(
  ...
  raterParamsUnfilteredMultiScorersList = [
    df for df in raterParamsUnfilteredMultiScorersList
...
def run_contributor_scoring(
  ...
  with c.time_block("Post-scorers: Compute helpfulness scores"):
    ...
    if strictColumns:
      # kh added this to avoid error due to missing columns
      for col in c.raterModelOutputTSVColumns: 
        if col not in helpfulnessScores.columns:
