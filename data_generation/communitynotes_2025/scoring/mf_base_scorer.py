...
class MFBaseScorer(Scorer):
  ...
  def __init__(
    ...
    minNumRatingsPerRater: int = 1,#10, # changed by YA
    minNumRatersPerNote: int = 1,#5, #changed by YA
    minRatingsNeeded: int = 1,#5,#changed by YA
...
def run_single_round_mf(
    ratingsForTraining: pd.DataFrame,
    userEnrollmentRaw: pd.DataFrame,
    modelingGroupToInitializeForStability: int = 13,
    minPercentRatingsFromModelingGroup: float = 0.75,
    minNumRatingsToIncludeInStableInitialization: int = 5,
    cutoffTimestampMillis: Optional[int] = None,  # <-- explicitly add this
    seed: Optional[int] = None
):
    if seed is not None:
    ...
    if cutoffTimestampMillis is not None:
        ratingsForTraining = ratingsForTraining[
            ratingsForTraining[c.createdAtMillisKey] <= cutoffTimestampMillis
        ]

    ratingsForStableInitialization = get_ratings_for_stable_init(
        ratingsForTraining, userEnrollmentRaw,
        modelingGroupToInitializeForStability,
        minPercentRatingsFromModelingGroup,
        minNumRatingsToIncludeInStableInitialization
    )

    mfRanker = MatrixFactorization(seed=seed)
    noteParams, raterParams, globalIntercept = mfRanker.run_mf(ratingsForStableInitialization)

    return noteParams, raterParams, globalIntercept
...
def run_single_round_mf(
    ...
    minNumRatingsToIncludeInStableInitialization: int = 5,
    seed: Optional[int] = None
)    Run only one round of matrix factorization (MF) on stable initialization data.

    Args:
        ratingsForTraining (pd.DataFrame): Ratings data.
        userEnrollmentRaw (pd.DataFrame): User enrollment data.
        modelingGroupToInitializeForStability (int): Group ID for initialization.
        minPercentRatingsFromModelingGroup (float): Minimum percentage of ratings required from the modeling group.
        minNumRatingsToIncludeInStableInitialization (int): Minimum ratings required to include a note in stable initialization.
        seed (Optional[int]): Seed for reproducibility.

    Returns:
        noteParams (pd.DataFrame): Note parameters from the MF.
        raterParams (pd.DataFrame): Rater parameters from the MF.
    ...
    # Instantiate and run matrix factorization once
    mfRanker = MatrixFactorization(seed=seed)
    noteParams, raterParams, globalIntercept = mfRanker.run_mf(ratingsForStableInitialization)
