...
class PFlipPlusModel(object):
  def __init__(
    ...
    helpfulnessRaterMin: int = 1,#changed
    tagRaterMin: int = 1, #changed
  ...
  def _compute_scoring_cutoff(
    self, noteStatusHistory: pd.DataFrame, ratings: pd.DataFrame, mode: str, minRatings: int = 1
  ) -> pd.DataFrame:
    ...
    print("len ratings 1",ratings.shape)
    cutoffByRatings = (
    ...
    print("len ratings 2",cutoffByRatings.shape)
    ...
    print("len ratings 3",cutoffByRatings.shape)
    ...
    # yegi added
    scoringCutoff[c.noteIdKey] = scoringCutoff[c.noteIdKey].astype("int64")
    cutoffByRatings[c.noteIdKey] = cutoffByRatings[c.noteIdKey].astype("int64")
    # yegi added finish
    scoringCutoff = scoringCutoff.merge(cutoffByRatings[[c.noteIdKey, "ratingMin"]],#how="left"
                                       )
    if ~(len(scoringCutoff) == beforeMerge):     # yegi added
        print("Debug",len(scoringCutoff),beforeMerge,scoringCutoff.shape, cutoffByRatings.shape)
    # assert len(scoringCutoff) == beforeMerge     # yegi removed
  ...
  def _prepare_note_info(
    ...
    if prepareForTraining:
      ...
      assert len(scoredNotes) == len(scoringCutoff)
        #yegi added start
      print("are labels a subset?",len(labels),len(scoringCutoff.merge(labels)))
      missing = labels[~labels[c.noteIdKey].isin(scoringCutoff[c.noteIdKey])]
      print("Missing notes in scoringCutoff:", len(missing))
      print(missing[c.noteIdKey].tolist())
        # yegi added finish
  ...
  def fit(
    ...
    print("debug, trainshape", trainDataFrame.shape)
