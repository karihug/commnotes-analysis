...
def read_from_tsv(
  ...
  #ya added
  if 'isMediaNote' in notes.columns:
            print("Removing unexpected column 'isMediaNote'")
            notes = notes.drop(columns=['isMediaNote'])
  if 'ratedOnTweetId' in ratings.columns:
        print("Removing unexpected column 'ratedOnTweetId'")
        ratings = ratings.drop(columns=['ratedOnTweetId'])
  extra_cols = set(userEnrollment.columns) - set(c.userEnrollmentTSVColumns)
  if extra_cols:
        print(f"Removing unexpected columns from userEnrollment: {extra_cols}")
        userEnrollment = userEnrollment.drop(columns=extra_cols)
...
def write_all_outputs(
 # noteParams: pd.DataFrame,
  ...
  newNoteStatusHistory: pd.DataFrame,
  auxilaryNoteInfo: pd.DataFrame,
  rater_df: pd.DataFrame
) -> None:
  
  #output_dir = c.scoredNotesOutputPath
  helpfulnessScores.to_csv("helpfulness_scores.tsv", sep="\t", index=False)
  newNoteStatusHistory.to_csv("note_status_history.tsv", sep="\t", index=False)
  auxilaryNoteInfo.to_csv("auxiliary_note_info.tsv", sep="\t", index=False)
  rater_df.to_csv( "rater_model_output.tsv", sep="\t", index=False)
